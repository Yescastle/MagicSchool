...
public class PlayerController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            if (!PlayerPrefs.HasKey("PlayerX"))
                transform.position = new Vector3(-16.63f, -2.66f, 0);
            else
            {
                float px = PlayerPrefs.GetFloat("PlayerX");
                float py = PlayerPrefs.GetFloat("PlayerY");
        ...
        if (SceneManager.GetActiveScene().name == "Classroom" || SceneManager.GetActiveScene().name == "Library")
        {
            canMove = false;
        }
    ...
    private void Update()
    {
        ...
        if (arrivePoint != null)
        {
            ...
            // x좌표가 플레이어보다 왼쪽에 있다면?
            if (goingPoint.x < current.x) Left();

            // x좌표가 플레이어보다 오른쪽에 있다면?
            else if (goingPoint.x > current.x) Right();

            if (goingPoint.y < current.y) Front();
    ...
    void Left()
    {
        move = PlayerMove.LeftMove;
        anim.SetTrigger("ToLeft");
    }

    void Right()
    {
        move = PlayerMove.RightMove;
        anim.SetTrigger("ToRight");
    }

    void Front()
    {
        move = PlayerMove.FrontMove;
        anim.SetTrigger("ToFront");
    }
}
